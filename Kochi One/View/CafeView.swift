...
// Main CafeView function that handles search integration
func CafeView(activeSheet: Binding<BottomBarView.SheetType?>, locationService: LocationService, restaurantService: RestaurantService, selectedRestaurantName: Binding<String?>) -> some View {
    CafeViewM(restaurantService: restaurantService, locationService: locationService, selectedRestaurantName: selectedRestaurantName)
}

// --- The original post template view ---
struct CafeViewFull: View {
...
struct CafeViewM: View {
    ...
    var body: some View {
        ...
        ScrollView(.vertical, showsIndicators: false) {
            LazyVStack(spacing: 0) { // spacing: 0 to remove extra padding between the CafeViewFull content
                if restaurantService.isLoading {
                    ...
                    ForEach(restaurantService.restaurants.filter { $0.isActive && $0.restaurantType?.lowercased() == "cafe" }) { restaurant in
                        CafeViewFull(showDetail: $showDetail, restaurant: restaurant, locationService: locationService)
                            .onTapGesture {
                                print("Cafe tapped: \(restaurant.name)")
        ...
        .fullScreenCover(isPresented: $showDetail) {
            if let restaurant = selectedRestaurant {
                CafeDetailView(restaurant: restaurant, locationService: locationService) {
                    // Back button action
                    showDetail = false
                ...
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if !restaurant.coverImages.isEmpty {
                        ...
                        ImageViewer(config: config) {
                            ...
                            OverlayViewCafe(activeID: nil, restaurant: restaurant)
                        } updates: { isPresented, activeID in
